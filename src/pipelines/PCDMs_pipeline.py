...
class ImageProjection(torch.nn.Module):
    ...
    def __init__(self, cross_attention_dim=768, clip_embeddings_dim=512, num_tokens=4) -> None:
        super().__init__()
        ...
        self.proj = torch.nn.Sequential(
            ...
            torch.nn.GELU(),
            torch.nn.Linear(clip_embeddings_dim * 2, cross_attention_dim * num_tokens),
        )
...
class PCDMsPipeline(
    ...
    def __call__(
        self,
        simg_mask_latents: Union[str, List[str]] = None,
        mask: Union[str, List[str]] = None,
        cond_pose: Union[str, List[str]] = None,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                latent_model_input = torch.cat([latents, mask, simg_mask_latents], dim=1).to(dtype=torch.float16)
